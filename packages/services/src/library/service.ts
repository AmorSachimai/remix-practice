...
export class LibraryService {
   ...
   * @returns
   */
  async findBookById(bookId: string): Promise<BookObject | undefined> {
   ...
   * @returns 書籍一覧
   */
  async findAll(volume = 20, offset = 0): Promise<BookObject[]> {
   ...
   * @returns 書籍情報
   */
  async findBookByISBN(isbn: string): Promise<BookObject | undefined> {
    if (!isISBN(isbn)) {
      return undefined;
    }
